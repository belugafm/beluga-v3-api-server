...
export default defineMethod(facts, argumentSpecs, expectedErrorSpecs, async (args, errors, authUser): ReturnType => {
    try {
        const channels = await new ChannelQueryRepository().list(getSortBy(args.sort_by), getSortOrder(args.sort_order))
        return channels.map((channel) => channel.toJsonObject())
    } catch (error) {
